...
CXXFLAGS += -O3 -g -std=c++11 -fopenmp -fpermissive -fmessage-length=0 -DHLS_NO_XIL_FPO_LIB
# Thirparty flags
...
--advanced.prop kernel.integrateKernel.kernel_flags="-std=c++0x -fexceptions -DHLS_NO_XIL_FPO_LIB" -g \
--advanced.prop kernel.bilateralFilterKernel.kernel_flags="-std=c++0x -fexceptions -DHLS_NO_XIL_FPO_LIB" \
--advanced.prop kernel.trackKernel.kernel_flags="-std=c++0x -fexceptions -DHLS_NO_XIL_FPO_LIB"
...
EXECUTABLE = host
TRAJ_PARAMS := -s 4.8 -p 0.34,0.5,0.24 -z 4 -c 2 -r 1 -k 481.2,480,320,240 -y 2
...
ifneq ($(HOST_ARCH), x86)
	$(ECHO) 'export PLATFORM_REPO_PATHS=/opt/Xilinx/Vitis/2019.2/platforms/'
	mkdir -p $(SDCARD)/$(BUILD_DIR)
	$(CP) $(B_NAME)/sw/$(XSA)/xrt/image/* $(SDCARD)
	$(CP) $(B_NAME)/sw/$(XSA)/boot/generic.readme xrt.ini $(EXECUTABLE) $(SDCARD)
	$(CP) $(XCLBIN)/*.xclbin $(SDCARD)/$(BUILD_DIR)/
ifeq ($(TARGET),$(filter $(TARGET),sw_emu hw_emu))
