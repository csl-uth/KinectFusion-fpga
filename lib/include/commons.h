...
struct Volume {
	...
	float interp(const float3 & pos) const {
		...
		float one = vs2(lower.x, lower.y, lower.z);
		float eight = vs2(upper.x, upper.y, upper.z);
		
		return (((one * (1 - factor.x)
					+ one * factor.x) * (1 - factor.y)
				+ (one * (1 - factor.x)
					+ one * factor.x) * factor.y) * (1 - factor.z)
				+ ((eight * (1 - factor.x)
						+ eight * factor.x)* (1 - factor.y)
					+ (eight * (1 - factor.x)
					+ eight * factor.x)* factor.y) * factor.z
				) * 0.00003051944088f;
...
inline __host__      __device__ floatH3 operator*(const Matrix4H & M,
		const floatH3 & v) {
