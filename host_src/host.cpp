...
int main(int argc, char ** argv) {
	...
	uint16_t* firstInputDepth = (uint16_t*) malloc(
	...
	*logstream
	...
	reader->readNextDepthFrame(firstInputDepth);
	...
	while (reader->readNextDepthFrame(inputDepth)) {
		...
		timings[1] = tock();
		if (frame == 0)
			kfusion.preprocessing(firstInputDepth, inputSize);
		...
		bool integrated = kfusion.integration(inputDepth, inputSize, camera, config.integration_rate,
				config.mu, frame);
