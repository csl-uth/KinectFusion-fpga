...
void Kfusion::languageSpecificConstructor() {
	...
	if (!krnl_integrateKernel || err != CL_SUCCESS) {
		printf("Error: Failed to create compute krnl_integrateKernel! %d\n", err);
		exit(EXIT_FAILURE);
	}
	...
	pyramidSz[0].x = computationSize.x;
	pyramidSz[0].y = computationSize.y;

	for (int i = 1; i < iterations.size(); i++) {
		pyramidSz[i].x = pyramidSz[i-1].x/2;
		pyramidSz[i].y = pyramidSz[i-1].y/2;
	}
	...
	for (unsigned int i = 1; i < iterations.size(); ++i) {
			ScaledDepth[i] = (float*) calloc(
					sizeof(float) * pyramidSz[i].x*pyramidSz[i].y, 1);
	}

	// 

	krnl_depth_size = sizeof(float) * computationSize.x * computationSize.y;
	
	padSize.x = computationSize.x + 4;
	padSize.y = computationSize.y + 4;
	...
	pad_depth = (float *)clEnqueueMapBuffer(q,floatDepth_buffer,true,CL_MAP_WRITE,0,krnl_paddepth_size,0,nullptr,nullptr,&err);
	...
	float gaussian_temp[5];
	int x;
	for (unsigned int i = 0; i < 5; i++) {
		x = i - 2;
		gaussian_temp[i] = expf(-(x * x) / (2 * delta * delta));
	}
	
	gaussian[0] = gaussian_temp[0]*gaussian_temp[0];
	gaussian[1] = gaussian_temp[0]*gaussian_temp[1];
	gaussian[2] = gaussian_temp[0]*gaussian_temp[2];
	gaussian[3] = gaussian_temp[0]*gaussian_temp[3];
	gaussian[4] = gaussian_temp[0]*gaussian_temp[4];
	gaussian[5] = gaussian_temp[1]*gaussian_temp[1];
	gaussian[6] = gaussian_temp[1]*gaussian_temp[2];
	gaussian[7] = gaussian_temp[1]*gaussian_temp[3];
	gaussian[8] = gaussian_temp[1]*gaussian_temp[4];
	gaussian[9] = gaussian_temp[2]*gaussian_temp[2];
	gaussian[10] = gaussian_temp[2]*gaussian_temp[3];
	gaussian[11] = gaussian_temp[2]*gaussian_temp[4];
	gaussian[12] = gaussian_temp[3]*gaussian_temp[3];
	gaussian[13] = gaussian_temp[3]*gaussian_temp[4];
	gaussian[14] = gaussian_temp[4]*gaussian_temp[4];
    ...
    integrate_depth_buffer = clCreateBuffer(context,  CL_MEM_READ_ONLY,  krnl_depth_size, NULL, &err);
	if (err != CL_SUCCESS) {
		std::cout << "Return code for clCreateBuffer - integrate_depth_buffer" << err << std::endl;
    }
	floatDepth = (float *)clEnqueueMapBuffer(q,integrate_depth_buffer,true,CL_MAP_WRITE,0,krnl_depth_size,0,nullptr,nullptr,&err);
...
Kfusion::~Kfusion() {
	...
	clReleaseMemObject(integrate_depth_buffer);
	

	clReleaseMemObject(integrate_vol_buffer);

	err = clEnqueueUnmapMemObject(q,floatDepth_buffer,pad_depth,0,NULL,NULL);
	if(err != CL_SUCCESS){
		printf("Error: Failed to unmap device memory floatDepth_buffer!\n");
	}
...
void bilateralFilterKernel(uint size_x, uint size_y, float e_d, int r){
	...
	err = clEnqueueMigrateMemObjects(q,(cl_uint)2,pt_bfKrnl, 0 ,0,NULL, NULL);
	...
	err = 0;
	err |= clEnqueueMigrateMemObjects(q,(cl_uint)1,&pt_bfKrnl[2], CL_MIGRATE_MEM_OBJECT_HOST,0,NULL, NULL);
	if (err != CL_SUCCESS) {
...
void integrateKernel(Volume vol, float* depth, uint2 depthSize,
	...
	pt_in[1] = volDim_buffer;
	pt_in[2] = integrate_depth_buffer;
	pt_in[3] = InvTrack_data_buffer;
	pt_in[4] = K_data_buffer;
	...
	err |= clSetKernelArg(krnl_integrateKernel,argcounter++, sizeof(int), &offset);
	err |= clSetKernelArg(krnl_integrateKernel,argcounter++, sizeof(int), &end);
	
	
	err |= clEnqueueMigrateMemObjects(q,(cl_uint)5, pt_in, 0 ,0,NULL, NULL);
	if (err != CL_SUCCESS) {
...
bool Kfusion::preprocessing(const ushort * inputDepth, const uint2 inputSize) {
	...
	for( int y = 0; y < computationSize.y; y++){
		for(int x = 0; x < computationSize.x; x++){
			pad_depth[x+2 + (y+2)*(padSize.x)] = floatDepth[x + y*computationSize.x];
		}
	}
