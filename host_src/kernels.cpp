...
// bilateral filter kernel variables
...
cl_mem integrate_vol_sbuffer[CU_CHUNK];
cl_event integrate_vol_events[CU_CHUNK];
...
cl_mem pt_trKrnl_in[6];
cl_mem pt_trKrnl_out[1];
...
void Kfusion::languageSpecificConstructor() {
	...
	krnl_depth_size = sizeof(floatH) * computationSize.x * computationSize.y;
	floatDepth = (float*) calloc(sizeof(float) * computationSize.x * computationSize.y, 1);
	
	padSize.x = computationSize.x + 2;
	padSize.y = computationSize.y + 2;
	...
	pt_bfKrnl[0] = floatDepth_buffer;
	pt_bfKrnl[1] = gaussian_buffer;
	pt_bfKrnl[2] = scaledDepth_zero_buffer;
	...
	size_t sub_krnl_vol_size = volumeResolution.x * volumeResolution.y *(volumeResolution.z/CU_CHUNK)*sizeof(short2);
	size_t matrix_sz = 16*sizeof(floatH);

	for (int i = 0; i < CU_CHUNK; i++) {
	...
	for (int i = 0; i < CU; i++) {
	    integrate_depth_buffer[i] = clCreateBuffer(context,  CL_MEM_READ_ONLY,  krnl_depth_size, NULL, &err);
		if (err != CL_SUCCESS) {
			std::cout << "Return code for clCreateBuffer - integrate_depth_buffer[" << i << "]: " << err << std::endl;
	    }
		floatHDepth[i] = (floatH *)clEnqueueMapBuffer(q,integrate_depth_buffer[i],true,CL_MAP_WRITE,0,krnl_depth_size,0,nullptr,nullptr,&err);

	    volSize_buffer[i] = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(uint4), NULL, &err);
		if (err != CL_SUCCESS) {
		...
		volDim_buffer[i] = clCreateBuffer(context, CL_MEM_READ_ONLY,  sizeof(floatH4), NULL, &err);
		if (err != CL_SUCCESS) {
		...
		InvTrack_data_buffer[i] = clCreateBuffer(context, CL_MEM_READ_ONLY, matrix_sz, NULL, &err);
		if (err != CL_SUCCESS) {
		...
		K_data_buffer[i] = clCreateBuffer(context, CL_MEM_READ_ONLY, matrix_sz, NULL, &err);
		if (err != CL_SUCCESS) {
	...
	for (int i = 0; i < iterations.size(); i++) {
		...
		inputVertex[i] = (float4*)clEnqueueMapBuffer(q_tr, inVertex_buffer[i], true, CL_MAP_READ | CL_MAP_WRITE, 0, size, 0, nullptr, nullptr, &err);
		...
		inputNormal[i] = (float4*)clEnqueueMapBuffer(q_tr, inNormal_buffer[i], true, CL_MAP_READ | CL_MAP_WRITE, 0, size, 0, nullptr, nullptr, &err);
	...
	trackingResult = (float4 *)clEnqueueMapBuffer(q_tr,trackData_float_buffer,true,CL_MAP_READ | CL_MAP_WRITE,0,trackFloat_sz,0,nullptr,nullptr,&err);
	...
	for (int i = 0; i < CU; i++) {
		inSize[i]->x = volume.size.x;
		inSize[i]->y = volume.size.y;
		inSize[i]->z = volume.size.z;

		inDim[i]->x = volume.dim.x;
		inDim[i]->y = volume.dim.y;
		inDim[i]->z = volume.dim.z;
	}
...
Kfusion::~Kfusion() {
	...
	for (int i = 0; i < CU; i++) {
		err = clEnqueueUnmapMemObject(q,integrate_depth_buffer[i],floatHDepth[i],0,NULL,NULL);
		if(err != CL_SUCCESS){
		...
		clReleaseMemObject(volSize_buffer[i]);
		clReleaseMemObject(volDim_buffer[i]);
		clReleaseMemObject(InvTrack_data_buffer[i]);
		clReleaseMemObject(K_data_buffer[i]);

		clReleaseMemObject(integrate_depth_buffer[i]);
	}

	for (int i = 0; i < CU_CHUNK; i++)
		clReleaseMemObject(integrate_vol_sbuffer[i]);
	...
	err = clEnqueueUnmapMemObject(q_tr,trackData_float_buffer,trackingResult,0,NULL,NULL);
	if(err != CL_SUCCESS){
	...
	for (int i = 0; i < CU; i++) {
		clReleaseKernel(krnl_integrateKernel[i]);
	}
	clReleaseCommandQueue(q);
    clReleaseCommandQueue(q_bf);
    clReleaseCommandQueue(q_tr);
...
void bilateralFilterKernel(uint size_x, uint size_y, float e_d, int r){
	...
	err = clEnqueueMigrateMemObjects(q_bf,(cl_uint)2,pt_bfKrnl, 0 ,0,NULL, NULL);
	...
	err = 0;
	err |= clEnqueueMigrateMemObjects(q_bf,(cl_uint)1,&pt_bfKrnl[2], CL_MIGRATE_MEM_OBJECT_HOST,0,NULL, NULL);
	if (err != CL_SUCCESS) {
...
void new_reduce(int blockIndex, float * out, float4* J, const uint2 Jsize,
		const uint2 size) {
	...
	for (uint y = blockIndex; y < size.y; y += 8) {
		for (uint x = 0; x < size.x; x++) {
			...
			if (row1.x < 1) {
				// accesses sums[28..31]
				/*(sums+28)[1]*/sums29 += row1.x == -4 ? 1 : 0;
				/*(sums+28)[2]*/sums30 += row1.x == -5 ? 1 : 0;
				/*(sums+28)[3]*/sums31 += row1.x > -4 ? 1 : 0;

				continue;
			}
			// Error part
			/*sums[0]*/sums0 += row1.y * row1.y;
			...
			/*(sums+7)[0]*/sums7 += row1.z * row1.z;
			/*(sums+7)[1]*/sums8 += row1.z * row1.w;
			/*(sums+7)[2]*/sums9 += row1.z * row2.x;
			/*(sums+7)[3]*/sums10 += row1.z * row2.y;

			/*(sums+7)[4]*/sums11 += row1.z * row2.z;
			/*(sums+7)[5]*/sums12 += row1.z * row2.w;

			/*(sums+7)[6]*/sums13 += row1.w * row1.w;
			/*(sums+7)[7]*/sums14 += row1.w * row2.x;
			/*(sums+7)[8]*/sums15 += row1.w * row2.y;
			/*(sums+7)[9]*/sums16 += row1.w * row2.z;

			/*(sums+7)[10]*/sums17 += row1.w * row2.w;

			/*(sums+7)[11]*/sums18 += row2.x * row2.x;
			/*(sums+7)[12]*/sums19 += row2.x * row2.y;
			/*(sums+7)[13]*/sums20 += row2.x * row2.z;
			/*(sums+7)[14]*/sums21 += row2.x * row2.w;

			/*(sums+7)[15]*/sums22 += row2.y * row2.y;
			/*(sums+7)[16]*/sums23 += row2.y * row2.z;
			/*(sums+7)[17]*/sums24 += row2.y * row2.w;

			/*(sums+7)[18]*/sums25 += row2.z * row2.z;
			/*(sums+7)[19]*/sums26 += row2.z * row2.w;

			/*(sums+7)[20]*/sums27 += row2.w * row2.w;
...
bool preprocessingARM(const ushort * inputDepth, const uint2 inputSize, const uint2 computationSize) {
	...
	for(int y = 0; y < computationSize.y; y++) {
		for(int x = 0; x < computationSize.x; x++) {
			pad_depth[x+1 + (y+1)*(padSize.x)] = floatDepth[x + y*computationSize.x];
		}
	}

	#pragma omp parallel for   
	for( int y = 0; y < padSize.y; y++) {
		pad_depth[padSize.x*y] = pad_depth[1 + padSize.x*y];
		//pad_depth[1 + padSize.x*y] = pad_depth[2 + padSize.x*y];
		//pad_depth[padSize.x-2 + padSize.x*y] = pad_depth[padSize.x-3 + padSize.x*y];
		pad_depth[padSize.x-1 + padSize.x*y] = pad_depth[padSize.x-2 + padSize.x*y];
	}

	// bilateralFilterKernel(computationSize.x,computationSize.y,e_delta, radius);
	return true;
}
...
void integrateKernel(const ushort * inputDepth, const uint2 inputSize, Volume vol, float* depth, uint2 depthSize,
	...
	#pragma omp parallel for
	for (int i = 0; i < CU; i++) {
		pt_in[i][0] = volSize_buffer[i];
		pt_in[i][1] = volDim_buffer[i];
		pt_in[i][2] = integrate_depth_buffer[i];
		pt_in[i][3] = InvTrack_data_buffer[i];
		pt_in[i][4] = K_data_buffer[i];
	}
	...
	for (k = 0; k < CU; k++)
		for (int i = 0; i < depthSize.x*depthSize.y; i++)
			floatHDepth[k][i] = floatDepth[i];

	int startFlag_offset = 0;
	if (LOOP_STEP == 3) {
		startFlag_offset = CU/2 + 1;
	}
	...
	end = volume.size.z/CU_CHUNK;
	for (int z = 0; z < CU_CHUNK; z++) {
		...
		kernel_id = z % CU;
	  	...
	  	err = 0;
		err |= clSetKernelArg(krnl_integrateKernel[kernel_id], argcounter++, sizeof(cl_mem), &volSize_buffer[kernel_id]);
		err |= clSetKernelArg(krnl_integrateKernel[kernel_id], argcounter++, sizeof(cl_mem), &integrate_vol_sbuffer[z]);
		err |= clSetKernelArg(krnl_integrateKernel[kernel_id], argcounter++, sizeof(cl_mem), &integrate_vol_sbuffer[z]);
		err |= clSetKernelArg(krnl_integrateKernel[kernel_id], argcounter++, sizeof(cl_mem), &integrate_vol_sbuffer[z]);
		err |= clSetKernelArg(krnl_integrateKernel[kernel_id], argcounter++, sizeof(cl_mem), &integrate_vol_sbuffer[z]);
		err |= clSetKernelArg(krnl_integrateKernel[kernel_id], argcounter++, sizeof(cl_mem), &volDim_buffer[kernel_id]);
		err |= clSetKernelArg(krnl_integrateKernel[kernel_id], argcounter++, sizeof(cl_mem), &integrate_depth_buffer[kernel_id]);
		err |= clSetKernelArg(krnl_integrateKernel[kernel_id], argcounter++, sizeof(int), &depthSize.x);
		err |= clSetKernelArg(krnl_integrateKernel[kernel_id], argcounter++, sizeof(int), &depthSize.y);
		err |= clSetKernelArg(krnl_integrateKernel[kernel_id], argcounter++, sizeof(cl_mem), &InvTrack_data_buffer[kernel_id]);
		err |= clSetKernelArg(krnl_integrateKernel[kernel_id], argcounter++, sizeof(cl_mem), &K_data_buffer[kernel_id]);
		err |= clSetKernelArg(krnl_integrateKernel[kernel_id], argcounter++, sizeof(floatH), &mu_h);
		err |= clSetKernelArg(krnl_integrateKernel[kernel_id], argcounter++, sizeof(floatH), &maxweight_h);
		err |= clSetKernelArg(krnl_integrateKernel[kernel_id], argcounter++, sizeof(int), &offset);
		err |= clSetKernelArg(krnl_integrateKernel[kernel_id], argcounter++, sizeof(int), &end);
		err |= clSetKernelArg(krnl_integrateKernel[kernel_id], argcounter++, sizeof(int), &startFlag_krnl);
		
		
		if( z == 0 && kernel_id == 0 || z==1 && kernel_id == 1 || z==2 && kernel_id == 2 || z==3 && kernel_id == 3){ // The first time, migrate buffers
				err |= clEnqueueMigrateMemObjects(q,(cl_uint)5, pt_in[kernel_id], 0 ,0,NULL, NULL);
				if (err != CL_SUCCESS)
					printf("Error: Failed to set krnl_integrateKernel[%d] arguments! %d\n", kernel_id, err);
			}

		err = clEnqueueTask(q, krnl_integrateKernel[kernel_id], 0, NULL, &integrate_vol_events[z]);
		if (err) {
			printf("Error: Failed to execute kernel! %d\n", err);
			return EXIT_FAILURE;
		}
		
		err |= clEnqueueMigrateMemObjects(q,(cl_uint)1, &integrate_vol_sbuffer[z], CL_MIGRATE_MEM_OBJECT_HOST,1,&integrate_vol_events[z], NULL);
		if (err != CL_SUCCESS) {
			printf("Error: Failed to set krnl_integrateKernel[%d] arguments! %d\n", kernel_id, err);
	...
	preprocessingARM(inputDepth, inputSize, depthSize);
...
bool Kfusion::tracking(float4 k, float icp_threshold, uint tracking_rate,
	...
	for (int level = iterations.size() - 1; level >= 0; --level) {
		...
		for (int i = 0; i < iterations[level]; ++i) {
			...
			pt_trKrnl_in[0] = inVertex_buffer[level];
			pt_trKrnl_in[1] = inNormal_buffer[level];
			pt_trKrnl_in[2] = refVertex_buffer;
			pt_trKrnl_in[3] = refNormal_buffer;
			pt_trKrnl_in[4] = Ttrack_data_buffer;
			pt_trKrnl_in[5] = view_data_buffer;

			pt_trKrnl_out[0] = trackData_float_buffer;
			...
			err = clEnqueueMigrateMemObjects(q_tr,(cl_uint)6, pt_trKrnl_in, 0 ,0,NULL, NULL);
			...
			err = 0;
			err |= clEnqueueMigrateMemObjects(q_tr,(cl_uint)1, pt_trKrnl_out, CL_MIGRATE_MEM_OBJECT_HOST,0,NULL, NULL);
			if (err != CL_SUCCESS) {
		        printf("Error: Failed to write to source array: %d!\n", err);
		        return EXIT_FAILURE;
		    }
...
bool Kfusion::raycasting(float4 k, float mu, uint frame) {
	...
	err |= clSetKernelArg(krnl_bilateralFilterKernel, argcounter++, sizeof(cl_mem), &scaledDepth_zero_buffer);
	err |= clSetKernelArg(krnl_bilateralFilterKernel, argcounter++, sizeof(cl_mem), &floatDepth_buffer);
	err |= clSetKernelArg(krnl_bilateralFilterKernel, argcounter++, sizeof(uint),&computationSize.x);
	err |= clSetKernelArg(krnl_bilateralFilterKernel, argcounter++, sizeof(uint),&computationSize.y);
	err |= clSetKernelArg(krnl_bilateralFilterKernel, argcounter++, sizeof(cl_mem), &gaussian_buffer);
	err |= clSetKernelArg(krnl_bilateralFilterKernel, argcounter++, sizeof(int), &radius);
	err |= clSetKernelArg(krnl_bilateralFilterKernel, argcounter++, sizeof(int),&start);
	err |= clSetKernelArg(krnl_bilateralFilterKernel, argcounter++,sizeof(int), &end);
	...
	// Call raycast
	if (frame > 2 && frame % RAYCAST_RATE == 0) {
		raycastPose = pose;
...
bool Kfusion::integration(const ushort * inputDepth, const uint2 inputSize, float4 k, uint integration_rate, float mu,
		uint frame) {
	...
	if ((doIntegrate && ((frame % integration_rate) == 0)) || (frame <= 3)) {
		...
		integrateKernel(inputDepth, inputSize, volume, floatDepth, computationSize, inverse(pose),
...
void Kfusion::renderTrack(uchar4 * out, uint2 outputSize) {
	renderTrackKernel(out, NULL, outputSize);
}
